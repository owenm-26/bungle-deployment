...
class CSRFToken(object):
    ...
    @staticmethod
    def validate(request, token):
        if request.forms.get('csrf') != token:
            raise HTTPError(403, output="CSRF Attack Detected (bad or missing token)")        

class CSRFTokenHTTPOnly(object):
    name = "Token validation (HTTPOnly)"
    @staticmethod
    def init(request, response):
        token = request.get_cookie("csrf")
        if token is None:
            token = os.urandom(16).encode('hex')
        response.set_cookie("csrf", token, httponly=True)
        return token
    @staticmethod
    def formHTML(token):
        return "<input type='hidden' name='csrf' value='" + token + "'>"
...
class CSRFTokenSecure(object):
    name = "Token validation (Secure)"
    @staticmethod
    def init(request, response):
        token = request.get_cookie("csrf")
        if token is None:
            token = os.urandom(16).encode('hex')
        response.set_cookie("csrf", token, secure=True)
        return token
    @staticmethod
    def formHTML(token):
        return "<input type='hidden' name='csrf' value='" + token + "'>"
...
class CSRFReferer(object):
    ...
    def validate(request, token):
        ...
        if referer != "" and referer.find("http://128.31.27.145/") != 0:
...
csrfDefenses = [CSRFNone,CSRFToken, CSRFTokenHTTPOnly, CSRFTokenSecure]
